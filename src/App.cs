...
public sealed class App : IDnsApplication
{
    ...
    public async Task InitializeAsync(IDnsServer dnsServer, string? config)
    {
        ...
        _config =
            JsonSerializer.Deserialize<Config>(config, JsonOptions)
        ...
        {
            _config.FritzBoxWebUrl = _config
                .FritzBoxUrl.Replace(":49000", "", StringComparison.OrdinalIgnoreCase)
                .TrimEnd('/');

            configChanged = true;

            Log($"FritzBoxWebUrl complemented: {_config.FritzBoxWebUrl}");
        }
        ...
        if (configChanged)
        {
            _ = Task.Run(async () =>
            {
                ...
                {
                    Log($"Error saving app configuration: {ex}");
                }
            });
        }
    ...
    private async Task SaveAppConfigAsync()
    {
        ...
        JsonSerializerOptions saveOptions = new(JsonOptions) { WriteIndented = true };

        string appConfig = JsonSerializer.Serialize(_config, saveOptions);

        string apiUrl = $"{_config.TechnitiumApiUrl.TrimEnd('/')}/api/apps/config/set";
    ...
    private async Task WorkerAsync(CancellationToken cancellationToken)
    {
        ...
        using PeriodicTimer timer = new(TimeSpan.FromMinutes(_config.IntervalMinutes));
    ...
    private async Task SynchronizeAsync(CancellationToken cancellationToken)
    {
        ...
        List<ReservedLease> reservations = await GetTechnitiumReservationsAsync(
            techClient,
            cfg,
            cancellationToken
        );
        ...
        using HttpClient fritzClient = FritzBoxClient.CreateHttpClient(
            cfg.FritzUsername,
            cfg.FritzPassword
        );

        FritzBoxClient fritzBox = new(fritzClient, cfg);

        List<FritzBoxClient.FritzDevice> fritzDevices = await fritzBox.GetHostListAsync(
            cancellationToken
        );

        Log($"FRITZ!Box: {fritzDevices.Count} Devices.");

        Dictionary<string, FritzBoxClient.FritzDevice> byMac = fritzDevices
        ...
        foreach (ReservedLease lease in reservations)
        {
            ...
            if (!byMac.TryGetValue(mac, out FritzBoxClient.FritzDevice? fritz))
            {
                notFound++;

                Log($"NICHT GEFUNDEN: {lease.HostName} [{mac}] IP {lease.Address}");
            ...
            try
            {
                await fritzBox.SetFriendlyNameByMacAsync(mac, desiredName, cancellationToken);
            ...
            catch (Exception ex)
            {
                ...
                Log("  -> Error while modifying: " + ex.Message);
            }
        }
        ...
        if (cfg.DryRun)
            Log("TEST MODE active – NO changes were made.");
    ...
    )
    {
        ...
        try
        {
            ...
            foreach (ReservedLease lease in reservations)
            {
                ...
                if (!lanDevicesByMac.TryGetValue(mac, out FritzBoxClient.FritzLanDevice? device))
                {
                    continue;
                }
                ...
                string desiredComment = $"FritzBoxSync - FRITZ!Box: {device.Name}";
                ...
                foreach (string ip in desiredIpv6)
                {
                    ...
                    if (
                        !string.IsNullOrWhiteSpace(reverseName)
                        && !string.IsNullOrWhiteSpace(reverseZone)
                    )
                    {
            ...
            Log("--------------------------------------------");
            Log($"Stable IPv6 devices            : {stableDevices}");

            Log($"Without stable IPv6            : {noStableIpv6}");

            Log($"AAAA already correct           : {alreadyPresent}");

            Log($"AAAA checked                   : {aaaaChecked}");

            Log($"AAAA created                   : {added}");

            Log($"AAAA updated                   : {updated}");

            Log($"AAAA deleted                   : {deleted}");

            Log($"IPv6 errors                    : {errors}");

            Log($"IPv6 PTR Sync enabled         : {cfg.EnableIpv6PtrSync}");

            if (cfg.EnableIpv6PtrSync)
            {
                await SynchronizeIpv6PtrAsync(techClient, cfg, desiredPtrs, cancellationToken);
            }

            if (cfg.DryRun)
            {
                Log("IPv6 Test Mode: " + "NO changes were made..");
            }
        }
        catch (Exception ex)
        {
            Log("IPv6 synchronization failed: " + ex.GetType().Name + ": " + ex.Message);
        }
    }
    ...
    )
    {
        ...
        if (desired.Count == 0)
        {
            ...
            */
            await CleanupManagedIpv6ReverseZonesAsync(client, cfg, desired, cancellationToken);
        ...
        try
        {
            ...
            foreach (
                var zoneGroup in desired.GroupBy(
                    x => x.ReverseZone,
            ...
            )
            {
                ...
                List<Ipv6PtrDesired> zoneDesired = zoneGroup.ToList();

                if (
                    !await EnsureTechnitiumReverseZoneAsync(
                        client,
                        cfg,
                        reverseZone,
                        CreateIpv6ReverseNetworkCidr(zoneDesired[0].IpAddress),
                        cancellationToken
                    )
                )
                {
                ...
                foreach (Ipv6PtrDesired wanted in zoneDesired)
                {
                    ...
                    TechnitiumPtrRecord? current = await GetTechnitiumPtrRecordAsync(
                        client,
                        cfg,
                        reverseZone,
                        wanted.ReverseName,
                    ...
                    {
                        bool targetCorrect = string.Equals(
                            NormalizeDnsName(current.PtrName, cfg.TechnitiumDnsZone),
                            NormalizeDnsName(wanted.TargetName, cfg.TechnitiumDnsZone),
                            StringComparison.OrdinalIgnoreCase
                        );
                        ...
                        bool ttlCorrect = current.Ttl == cfg.Ipv6PtrTtl;

                        if (targetCorrect && commentCorrect && ttlCorrect)
                        {
                            alreadyCorrect++;
                            continue;
                        }
                        ...
                        else
                        {
                            ...
                            {
                                errors++;
                                Log($"PTR UPDATE ERROR: {wanted.TargetName} | {ex.Message}");
                            }
                        }
                    ...
                    if (cfg.DryRun)
                    {
                        Log($"PTR ADD (DRY RUN): {wanted.TargetName} | {wanted.IpAddress}");
                        added++;
                    }
                    else
                    {
                        try
                        {
                            ...
                            added++;
                            Log($"PTR ADD: {wanted.TargetName} | {wanted.IpAddress}");
                        }
                        catch (Exception ex)
                        {
                            errors++;
                            Log($"PTR ADD ERROR: {wanted.TargetName} | {ex.Message}");
                        }
                    }
                }
            }
            ...
            */
            ...
            )
            {
                ...
                HashSet<string> desiredOwners = zoneDesired
                    .Select(x => NormalizeReverseName(x.ReverseName))
                    .ToHashSet(StringComparer.OrdinalIgnoreCase);

                List<TechnitiumPtrRecord> zoneRecords = await GetTechnitiumPtrRecordsAsync(
                    client,
                    cfg,
                    reverseZone,
                    cancellationToken
                ...
                foreach (TechnitiumPtrRecord existing in zoneRecords)
                {
                    ...
                    if (
                        string.IsNullOrWhiteSpace(existing.Comments)
                    ...
                    string owner = NormalizeReverseName(existing.Name);
                    ...
                    if (cfg.DryRun)
                    {
                        Log($"PTR DELETE (DRY RUN): {existing.PtrName} | {existing.Name}");
                        continue;
                    }
                    ...
                    try
                    {
                        ...
                        deleted++;
                        Log($"PTR DELETE: {existing.PtrName} | {existing.Name}");
                    }
                    catch (Exception ex)
                    {
                        errors++;
        ...
        {
            Log("IPv6 PTR synchronization failed: " + ex.GetType().Name + ": " + ex.Message);
        }
    }

    private static string CreateIpv6ReverseName(string ipAddress)
    {
        if (
        ...
        string hex = Convert.ToHexString(address.GetAddressBytes()).ToLowerInvariant();

        int prefixNibbles = IsIpv6Ula(ipAddress) ? 14 : 16;
        ...
        return string.Join(".", hex.Substring(prefixNibbles).Reverse());
    }

    private static string CreateIpv6ReverseZone(string ipAddress)
    {
        if (
            !IPAddress.TryParse(ipAddress, out IPAddress? address)
            || address.AddressFamily != System.Net.Sockets.AddressFamily.InterNetworkV6
        ...
        return string.Join(".", hex.Substring(0, prefixNibbles).Reverse()) + ".ip6.arpa";
    }

    private static string CreateIpv6ReverseNetworkCidr(string ipAddress)
    {
        if (
            !IPAddress.TryParse(ipAddress, out IPAddress? address)
            || address.AddressFamily != System.Net.Sockets.AddressFamily.InterNetworkV6
        ...
        int prefixLength = IsIpv6Ula(ipAddress) ? 56 : 64;

        byte[] bytes = address.GetAddressBytes();

        for (int i = prefixLength / 8; i < bytes.Length; i++)
        {
            bytes[i] = 0;
        }
    ...
    {
        string listUrl = $"{cfg.TechnitiumApiUrl.TrimEnd('/')}" + "/api/zones/list";

        using HttpRequestMessage request = new(HttpMethod.Get, listUrl);
        ...
        if (ZoneExists(doc.RootElement, reverseZone))
        {
            /*
            ...
            */
            try
            {
                List<TechnitiumPtrRecord> existingRecords = await GetTechnitiumPtrRecordsAsync(
                    client,
                    cfg,
                    reverseZone,
                    cancellationToken
                ...
                bool hasManagedPtr = existingRecords.Any(record =>
                ...
                if (hasManagedPtr)
                {
                    if (cfg.DryRun)
                    {
                        Log($"PTR ZONE REGISTER (DRY RUN): {reverseZone}");
                    }
                    else if (AddManagedIpv6ReverseZone(cfg, reverseZone))
                    {
                        Log($"PTR ZONE REGISTER: {reverseZone}");
            ...
            {
                Log("PTR: could not inspect existing reverse zone: " + ex.Message);
            }
        ...
        if (cfg.DryRun)
        {
            Log($"PTR ZONE CREATE (DRY RUN): {reverseZone}");
            return true;
        }

        if (AddManagedIpv6ReverseZone(cfg, reverseZone))
        {
            Log($"PTR ZONE REGISTER: {reverseZone}");
            ScheduleAppConfigSave();
        }
        ...
        using HttpRequestMessage createRequest = new(HttpMethod.Get, createUrl);
        ...
        EnsureTechnitiumApiOk(createBody, "Technitium reverse zone CREATE");

        Log($"PTR ZONE CREATE: {reverseZone}");

        return true;
    }

    private static bool ZoneExists(JsonElement root, string zoneName)
    {
        if (!root.TryGetProperty("response", out JsonElement response))
            return false;

        if (
            !response.TryGetProperty("zones", out JsonElement zones)
        ...
        {
            string? name = GetString(zone, "name");
    ...
    private static async Task<List<TechnitiumPtrRecord>> GetTechnitiumPtrRecordsAsync(
        HttpClient client,
        Config cfg,
        string reverseZone,
        CancellationToken cancellationToken
    )
    {
        ...
        if (
            ...
            || !responseElement.TryGetProperty("records", out JsonElement records)
        ...
        {
            if (
                !string.Equals(GetString(record, "type"), "PTR", StringComparison.OrdinalIgnoreCase)
            )
            {
                continue;
            }
            ...
            if (
                record.TryGetProperty("rData", out JsonElement rData)
                && rData.ValueKind == JsonValueKind.Object
            )
            {
                ptrName = GetString(rData, "ptrName") ?? "";
            }

            string? comments = GetString(record, "comments");

            int ttl = 0;

            if (
                record.TryGetProperty("ttl", out JsonElement ttlElement)
            ...
            result.Add(
                new TechnitiumPtrRecord(name.TrimEnd('.'), ptrName.TrimEnd('.'), comments, ttl)
            );
        }
    ...
    )
    {
        ...
        {
            throw new InvalidOperationException("PTR DELETE without a PTR target is not allowed.");
        }
    ...
    )
    {
        ...
        foreach (JsonElement record in records.EnumerateArray())
        {
            ...
            return new TechnitiumPtrRecord(name.TrimEnd('.'), ptrName.TrimEnd('.'), comments, ttl);
        }
    ...
    )
    {
        ...
        EnsureTechnitiumApiOk(body, "Technitium PTR ADD");
    }
    ...
    )
    {
        ...
        EnsureTechnitiumApiOk(body, "Technitium PTR UPDATE");
    }

    private static void EnsureTechnitiumApiOk(string body, string operation)
    {
        using JsonDocument doc = JsonDocument.Parse(body);
        ...
        if (doc.RootElement.TryGetProperty("errorMessage", out JsonElement errorMessageElement))
        {
        ...
        throw new InvalidOperationException($"{operation} error: {errorMessage}");
    }

    private static List<string> GetStableIpv6(FritzBoxClient.FritzLanDevice device)
    {
    ...
    private static string CreateFritzPbkdf2Response(string challenge, string password)
    {
        string[] parts = challenge.Split('$');

        if (parts.Length != 5 || parts[0] != "2")
        {
            throw new FormatException("Invalid FRITZ!Box PBKDF2 challenge format.");
        }

        int iterations1 = int.Parse(parts[1]);

        string salt1Hex = parts[2];

        int iterations2 = int.Parse(parts[3]);

        string salt2Hex = parts[4];

        byte[] salt1 = Convert.FromHexString(salt1Hex);

        byte[] salt2 = Convert.FromHexString(salt2Hex);
        ...
        return Convert.ToHexString(salt2).ToLowerInvariant()
            + "$"
            + Convert.ToHexString(hash2).ToLowerInvariant();
    }

    private static async Task<List<TechnitiumRecord>> GetTechnitiumRecordsAsync(
        HttpClient client,
        Config cfg,
        CancellationToken cancellationToken
    )
    {
        string zone = cfg.TechnitiumDnsZone.Trim().TrimEnd('.');
        ...
        {
            throw new InvalidOperationException("Technitium API liefert keine records.");
        }
        ...
        foreach (JsonElement record in records.EnumerateArray())
        {
            ...
            {
                ipAddress = GetString(rData, "ipAddress");
            }
            ...
            result.Add(new TechnitiumRecord(name, type, ipAddress, comments, ttl));
        }
    ...
    )
    {
        if (string.IsNullOrWhiteSpace(existing.IpAddress))
        {
            throw new InvalidOperationException("AAAA update without an existing IP address.");
        }
    ...
    private static async Task<List<ReservedLease>> GetTechnitiumReservationsAsync(
        HttpClient client,
        Config cfg,
        CancellationToken cancellationToken
    )
    {
        string url = $"{cfg.TechnitiumApiUrl.TrimEnd('/')}/api/dhcp/leases/list";
        ...
        try
        {
            ...
            JsonElement leases = doc.RootElement.GetProperty("response").GetProperty("leases");

            List<ReservedLease> result = new();

            foreach (JsonElement lease in leases.EnumerateArray())
            {
                string? type = GetString(lease, "type");

                if (!string.Equals(type, "Reserved", StringComparison.OrdinalIgnoreCase))
                    continue;

                string? mac = GetString(lease, "hardwareAddress");

                string? ip = GetString(lease, "address");

                string? host = GetString(lease, "hostName");
                ...
                result.Add(new ReservedLease(mac, ip, host));
            }
    ...
    {
        return new string(mac.Where(char.IsLetterOrDigit).ToArray()).ToUpperInvariant();
    }

    private static string FormatMac(string normalized)
    {
        normalized = NormalizeMac(normalized);

        if (normalized.Length != 12)
        {
            throw new FormatException($"Ungültige MAC-Adresse: {normalized}");
        }
    ...
    {
        _dnsServer?.WriteLog("[FRITZ!Box Sync] " + message);
    }
    ...
    private static bool IsManagedIpv6ReverseZone(Config cfg, string reverseZone)
    {
        string normalized = NormalizeReverseName(reverseZone);

        return cfg.ManagedIpv6ReverseZones.Any(x =>
            string.Equals(NormalizeReverseName(x), normalized, StringComparison.OrdinalIgnoreCase)
        );
    }

    private static bool AddManagedIpv6ReverseZone(Config cfg, string reverseZone)
    {
        string normalized = NormalizeReverseName(reverseZone);
        ...
        if (IsManagedIpv6ReverseZone(cfg, normalized))
        {
            return false;
        }
    ...
    private static bool RemoveManagedIpv6ReverseZone(Config cfg, string reverseZone)
    {
        string normalized = NormalizeReverseName(reverseZone);

        int removed = cfg.ManagedIpv6ReverseZones.RemoveAll(x =>
            string.Equals(NormalizeReverseName(x), normalized, StringComparison.OrdinalIgnoreCase)
        );
    ...
    {
        if (Interlocked.Exchange(ref _configSaveScheduled, 1) != 0)
        {
            return;
        }
        ...
        _ = Task.Run(async () =>
        {
            ...
            {
                Interlocked.Exchange(ref _configSaveScheduled, 0);
            }
        });
    }
    ...
    )
    {
        ...
        foreach (JsonElement record in records.EnumerateArray())
        {
            ...
            if (
                string.Equals(type, "SOA", StringComparison.OrdinalIgnoreCase)
                || string.Equals(type, "NS", StringComparison.OrdinalIgnoreCase)
            )
            {
                continue;
            }
    ...
    internal sealed class Config
    {
        ...
        public string TechnitiumApiUrl { get; set; } = "http://192.168.178.2:5380";

        public string TechnitiumDnsZone { get; set; } = "koch.local";

        public string TechnitiumApiToken { get; set; } = "";
    }

    private sealed record ReservedLease(string HardwareAddress, string Address, string HostName);
